...
class InputTracker {
    ...
    add(name, pos) {
        if (name in this.#inputs) pos.copyTo(this.#inputs[name].basePos);
    ...
    applyOffset(offset) {
        if ('MouseMove' in this.#inputs) this.#inputs['MouseMove'].offset = offset;
        if ('MouseHold' in this.#inputs) this.#inputs['MouseHold'].offset = offset;
    }
}
